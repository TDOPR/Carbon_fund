...
public class TronEventManager {
    ...
    public void analyzeTRONEvent() throws Exception {
        ...
        CoinConfig scanDataConfig = coinConfigMapper.getScanDataConfig(Integer.valueOf(chainIdsConfig.getTron()));
        BigInteger blockNumber = scanDataConfig.getBlockNo();
        blockNumber = blockNumber.add(BigInteger.ONE);
        //获取当前最新区块
        String blockStr = doGet(tronConfig.getUrlLastBlock());
        ...
        if (currentBlock.compareTo(blockNumber.add(new BigInteger(tronConfig.getBlockDiff().toString()))) == 1) {
            ...
            for (int i = 0; i < events.size(); i++) {
                ...
                if (eventName.equals("TransferUsdt")) {
                    ...
                    donaUsersWalletsService.updateUsdWallet(rechargeAmount, appDonaUsersMapper.selectDonaUserIdByUserAddress(userAddress), FlowingActionEnum.INCOME, UsdLogTypeEnum.RECHARGE);
